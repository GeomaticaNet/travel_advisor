...
const MAP_STYLES = [
  { id: "streets", label: "Calles", url: "mapbox://styles/mapbox/streets-v11" },
  { id: "satellite", label: "Satélite", url: "mapbox://styles/mapbox/satellite-v9" },
  { id: "satellite-streets", label: "Satélite + Calles", url: "mapbox://styles/mapbox/satellite-streets-v11" },
  { id: "outdoors", label: "Exterior", url: "mapbox://styles/mapbox/outdoors-v11" },
  { id: "light", label: "Claro", url: "mapbox://styles/mapbox/light-v10" },
  { id: "dark", label: "Oscuro", url: "mapbox://styles/mapbox/dark-v10" },
];

const MapComponent = ({ setCoordinates, setBounds, coordinates, places, setChildClicked }) => {
  ...
  useEffect(() => {
    if (map.current) map.current.setStyle(mapStyle.url);
  }, [mapStyle]);
  ...
  useEffect(() => {
    ...
    places.forEach((place, i) => {
      ...
      if (!isDesktop) {
        ...
        el.innerHTML = '<div style="background:white;padding:10px;border-radius:4px;box-shadow:0 2px 6px rgba(0,0,0,0.3);width:100px;text-align:center"><div style="font-size:12px;font-weight:500;margin-bottom:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + place.name + '</div><img src="' + imgSrc + '" style="width:100%;height:60px;object-fit:cover;border-radius:2px" /><div style="margin-top:4px;color:#f5a623;font-size:12px">★ ' + Number(place.rating) + '</div></div>';
      }
  ...
  return (
    <div className={classes.mapContainer}>
      ...
      <div className={classes.styleSwitcher}>
        <div className={classes.styleButton} onClick={() => setShowStyles(!showStyles)}>
          <div className={classes.styleThumbMini} />
          <Typography variant="caption" className={classes.styleLabel}>Mapa</Typography>
        </div>
        {showStyles && (
